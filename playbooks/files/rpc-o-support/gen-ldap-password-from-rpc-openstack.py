...
#
# This script has not been tested fully
# Please do not use it unless you understand the implications
#
...
ldap_host = 'ldap://ldap'
ldap_base_dn = ("OU=OpenStack,"
                "OU=Service Accounts,"
                "OU=Administrative Accounts,"
                "DC=foo,DC=bar")
# Flip to one if you just want to get the output of what the script is doing
simulation = 0

# If initial_user_pwd is specified, the ldap bind will use this password
# to authenticate and update the password with the service_password
# from the user_secrets.yml and user_extras_secrets.yml
ldap_initial_bind_pwd = 'HolySmokes123'

# Where is my openstack-ansible (OSA) service username and password
...
# Define mappings for user/password keys
# Not yet included in RPC12:
# ceilometer_service_user_name': 'ceilometer_service_password'
osa_user_tokens = {'cinder_service_user_name': 'cinder_service_password',
...
def update_ldap_passwords_from_osa():
    ...
    for token in osa_user_tokens:
        ...
        vyml = dict(get_config(osa_variables_file).items() +
                    get_config(osa_variables_extras_file).items())
        syml = dict(get_config(osa_secrets_file).items() +
                    get_config(osa_secrets_extras_file).items())
        ...
        try:
            ...
            print("Service user ", service_user,
                  " password ", service_password)

            new_service_password = service_password
            ldap_new_password = ('"%s"' % new_service_password). \
                encode("utf-16-le")
            if not simulation:
                ldap_mod_attrs = [(ldap.MOD_REPLACE, 'unicodePwd',
                                   ldap_new_password)]
                ldap_con.modify_s('CN=%s,%s' % service_user,
                                  ldap_base_dn,
                                  ldap_mod_attrs)
            else:
                print("LDAP update simulated to update unicodePwd with ",
                      ldap_new_password)
        except Exception as e:
            print("Could not update user ", service_user,
                  " with token ",
                  token)
